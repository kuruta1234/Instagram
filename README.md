...
- キャプション・ハッシュタグは、**Claude Code CLI(`claude`コマンド)を呼び出して画像を見た上で自動生成**します。
  Anthropic APIキーは不要で、`claude` コマンドが既にログイン済み(Claude Pro/Max等のサブスクリプション、
  またはAPIキー)であればその認証情報がそのまま使われます。もちろん生成後にテキストを自由に編集して保存できます。
- 画像トリミングはGUIでは**中央固定ではなく、ドラッグで任意の範囲を選択でき、ズームイン・アウトも可能**です。
- 画像の自動補正・背景除去・透かし合成もまとめて扱えます。
...
キャプション自動生成には [Claude Code](https://claude.com/claude-code) がインストールされ、
`claude` コマンドでログイン済みであることが必要です。

```bash
npm install -g @anthropic-ai/claude-code
claude login   # Claude Pro/Max等のアカウントでログイン(APIキーでも可)
```
...
3. **画像編集ページ** — 画像上をドラッグして任意の範囲を選択してトリミング(中央固定ではありません)。マウスホイールやズームスライダーで拡大・縮小、アスペクト比ボタン(自由/1:1/4:5/1.91:1/9:16)や出力サイズ指定も可能。自動補正・背景除去・透かし・テキスト合成もここから実行できる
4. **エクスポート** — 画像とキャプションをまとめて書き出し、Instagramアプリから手動で投稿
...
| 画像編集 | `igtool edit` | Instagram向けリサイズ(中央クロップ)・自動補正・背景除去・透かし・テキスト合成 |
